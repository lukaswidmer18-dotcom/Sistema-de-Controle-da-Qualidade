...
function Section({ title, icon: Icon, children, adminOnly }: SectionProps) {
  return (
    <div className={`brand-card motion-enter rounded-xl p-6 space-y-4 ${adminOnly ? 'border-brand-gold/35' : ''}`}>
      <div className="flex items-center gap-2.5">
        <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${adminOnly ? 'bg-brand-gold/16' : 'bg-brand-green/10'}`}>
          <Icon className={`w-4.5 h-4.5 ${adminOnly ? 'text-brand-gold' : 'text-brand-green'}`} />
        </div>
        <h2 className="text-base font-bold text-brand-green">{title}</h2>
        {adminOnly && (
...
export default function GuiaMetricasPage() {
    ...
    <div className="min-h-screen brand-shell">
      <div className="brand-header px-6 py-4 sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 brand-icon-gold rounded-lg flex items-center justify-center shrink-0">
            <BookOpen className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">Guia de Métricas</h1>
            <p className="text-sm text-white/65">Como interpretar os status e cores do sistema</p>
          </div>
        </div>
      </div>
